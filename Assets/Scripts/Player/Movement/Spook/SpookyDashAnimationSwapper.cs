...
public class SpookyDashAnimationSwapper : MonoBehaviour
{
    ...
    [SerializeField] private PlayerDash playerDash;   // your dash
    [SerializeField] private SpookyDash spookyDash;   // tells us which dash is spooky
    ...
    [SerializeField] private bool includePhaseGrace = true;

    [Tooltip("Safety: force-restore after this many seconds post spooky end. 0 = no timeout.")]
    [SerializeField] private float postDashRestoreTimeout = 1.0f;
    ...
    [SerializeField] private bool preExitEaseEnabled = true;
    ...
    [SerializeField] private bool softLandingEnabled = true;
    ...
    private void Update()
    {
        // Only start spooky overrides if THIS dash is spooky.
        bool spookyActiveNow =
            includePhaseGrace ? spookyDash.IsCurrentlySpookyDashing()
                              : (playerDash.IsDashing() && spookyDash.IsCurrentlySpookyDashing());
        ...
        // When spookyActive ends, ARM a restore, but let spooky clips finish blending out
        ...
        if (_restoreArmed)
        {
            // Optional: stretch the first detected transition to next state
    ...
    /// Returns true if any of the given clips are playing (current or next on any layer).
    ...
    /// Softens the jump into Run/Idle by re-issuing the controller’s chosen next state
    /// with a slightly longer crossfade (once per spooky cycle).
    private void TrySoftLandingCrossfade()
    {
        ...
        for (int layer = 0; layer < layers; layer++)
        {
            // Only if controller already queued a next state
            var nextInfo = animator.GetNextAnimatorStateInfo(layer);
            if (nextInfo.fullPathHash != 0 && nextInfo.length > 0f)
            {
                // Re-crossfade to the same next state with our duration
    ...
    /// While spooky clips are active → boost speed.
    /// Near the end of spooky clips → gradually reduce to a landing multiplier.
    /// After spooky ends → ease back to base over speedBlendOutTime.
    private void DriveAnimatorSpeed(bool spookyActiveNow)
    {
        ...
        if (spookyPlayingNow)
        {
            ...
            // Pre-exit easing: look for any CURRENT state using a spooky clip and check normalized time
            if (preExitEaseEnabled && preExitLandingMultiplier >= 1f)
            {
                ...
                for (int layer = 0; layer < layers; layer++)
                {
                    ...
                    // Only consider if state has weight (avoid partial blends skewing result)
                    if (animator.GetLayerWeight(layer) < 0.5f) continue;

                    // If any current clip is one of the spooky clips
                    ...
                    // Normalized time in [0..1) for looping states; for non-looping this still rises >1,
                    // so use frac part to detect “where in the loop/clip” we are.
