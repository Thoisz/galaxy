...
public class PlayerDash : MonoBehaviour
{
    ...
    // NEW: explicit guard so altitude maintenance only runs when initialized
    private bool _altitudeLockActive = false;
    ...
    private void StartDash()
    {
        ...
        _isDashing = true;
        _isDashStopping = false; // Reset slowing state
        _canDash = false;
        _canChangeDirection = false;
        _directionChangeTimer = 0f;
        _hasInputDirection = false;
        _hasJumpedDuringDash = false;
        _altitudeLockActive = false; // will be enabled if airborne baseline is set
        ...
        // Initialize altitude baseline ONLY if airborne and gravity is valid
        _altitudeLockActive = !_dashStartedGrounded && _gravityDirection != Vector3.zero;
        if (_altitudeLockActive)
        {
            _dashStartAltitude = Vector3.Dot(transform.position, -_gravityDirection);
        }
        else
        {
            _dashStartAltitude = 0f;
        }
        ...
        if (!_isDashStopping)
        {
            ...
            if (shouldSync)
            {
                ...
                if (cameraForward.sqrMagnitude > 0.01f && cameraRight.sqrMagnitude > 0.01f)
                {
                    ...
                    // Apply new velocity direction even when jumped during dash
    ...
    private void ApplyDashVelocityForCameraPanning()
    {
        ...
        // Special case for camera panning during jump
        ...
        if (_hasJumpedDuringDash)
        {
            ...
            // Use dash speed instead of preserving current horizontal speed
    ...
    private void MaintainSlowdownState()
    {
        ...
        // Keep vertical component for potential altitude maintenance
        Vector3 verticalVelocity = Vector3.Project(currentVelocity, gravityUp);

        // For airborne dashes: maintain exact altitude during slowdown (only if initialized)
        if (_altitudeLockActive)
        {
    ...
    private void ApplyDashVelocity()
    {
        ...
        // Don't apply dash velocity when jumped during dash
        if (_hasJumpedDuringDash)
        {
            return;
        }
    ...
    private void MaintainDashState()
    {
        ...
        // Don't maintain velocity at all when jumped during dash
        if (_hasJumpedDuringDash)
        {
        ...
        // Normal dash state maintenance (lock speed to dash speed)
        ...
        // For airborne dashes: maintain exact altitude (only when initialized)
        if (_altitudeLockActive)
        {
            float currentAltitude = Vector3.Dot(transform.position, -_gravityDirection);
            float altitudeDifference = currentAltitude - _dashStartAltitude;
    ...
    private void EndDash()
    {
        ...
        // disable altitude lock explicitly
        _altitudeLockActive = false;
        ...
        // Handle velocity based on whether there was input during the dash
        if (_rigidbody != null)
        {
            Vector3 currentVelocity = _rigidbody.velocity;
            Vector3 gravityUp = GetCurrentUpVector();
