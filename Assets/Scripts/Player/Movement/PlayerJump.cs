...
public class PlayerJump : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (blockJumpNow)
        {
            // Animator: show last fall on ground; live timer while actually falling
            if (_animator != null)
            {
                if (isGrounded)               _animator.SetFloat("fallTime", _lastFallTime);
    ...
    private void TryJumpFromDash()
    {
        ...
        // Reset animator fall clock immediately on jump
        if (_animator != null) _animator.SetFloat("fallTime", 0f);
    ...
    private void PerformJumpWithoutEndingDash(float force, float cooldown, AudioClip sound)
    {
        ...
        // Reset fall timers + animator **immediately** on jump
        _lastFallTime = 0f;
        _currentFallTime = 0f;
        _isFalling = false;
        _wasActuallyFallingLastFrame = false;
    ...
    private void UpdateFallTracking(bool isGrounded)
    {
        ...
        {
            // On ground: freeze & store last fall duration (keep for animator on ground)
            if (_isFalling)
            {
    ...
    private void ResetJumps()
    {
        ...
        // NOTE: We still do not reset _lastFallTime here.
        // It now persists on ground for the animator until the next jump.
    }
