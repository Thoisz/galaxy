...
{
    if (_showDebugLogs)
        Debug.Log("ActivateSuperSpeed called");

    // Check if speed is sufficient to activate super speed
    if (_currentTargetSpeed < _minSpeedForSuperSpeedActivation)
    {
        if (_showDebugLogs)
            Debug.Log($"Super speed activation blocked — current speed {_currentTargetSpeed:F2} is below required {_minSpeedForSuperSpeedActivation}");
        return;
    }
    ...
    {
        if (_showDebugLogs)
            Debug.Log("Setting animator parameter isSuperSpeed to true");
    ...
    {
        if (!_useScreenEffects || _mainCamera == null)
            return;
    ...
    private bool CheckForGrounding()
    {
        ...
        Vector3 downDirection = _gravityBody != null ? 
    ...
    private void ApplyFlightMovement()
{
    ...
    if (_hasMovementInput)
    {
        ...
        float lerpFactor = _isSuperSpeedActive ? 
    ...
    else
    {
        ...
        if (!_isInGravityTransition)
        {
            ...
            Vector3 upDirection = _isInSpace ? _spaceUpVector : 
    ...
    private void RotateTowardsFlyingDirection()
{
    ...
    Vector3 stableUp = _isInSpace ? _spaceUpVector :
        (_gravityBody != null ? -_gravityBody.GravityDirection.normalized : Vector3.up);
    ...
    else
    {
        ...
        if (_isInSpace)
        {
            lookDir = (_playerCamera != null)
                ? _playerCamera.GetCameraForward()
                : (_cameraTransform != null ? _cameraTransform.forward : transform.forward);

            upDir = (_playerCamera != null)
                ? _playerCamera.GetCameraUp()
                : (_cameraTransform != null ? _cameraTransform.up : Vector3.up);

            targetRotation = Quaternion.LookRotation(lookDir, upDir);
        }
        else
        {
            Vector3 fallbackForward = (_playerCamera != null)
                ? _playerCamera.GetCameraForward()
                : (_cameraTransform != null ? _cameraTransform.forward : transform.forward);
    ...
    if (_isRecoveringFromTransition)
        rotationFactor *= 2f;
    ...
    private void UpdateFlightGravityInteraction()
{
    ...
    Vector3 gravityDirection = _gravityBody != null && _gravityBody.enabled
        ? _gravityBody.GravityDirection.normalized
    ...
    if (_isDrifting && _isInSpace)
    {
        ...
        if (_playerCamera != null)
        {
            ...
            if (forward.sqrMagnitude < 0.01f)
                forward = transform.forward; // fallback if projection fails
...
public void OnGravityTransitionCompleted()
{
    ...
    {
        Vector3 newDirection = _gravityBody.GravityDirection;
        Vector3 oldDirection = _previousGravityDirection != Vector3.zero ? 
                               _previousGravityDirection : 
                               newDirection;
        float defaultDuration = 0.5f;
    ...
    void OnDrawGizmosSelected()
    {
        ...
        if (_cameraTransform != null && Application.isEditor)
        {
            ...
            Vector3 upVector = _gravityBody != null ? 
                -_gravityBody.GravityDirection.normalized : 
                Vector3.up;
