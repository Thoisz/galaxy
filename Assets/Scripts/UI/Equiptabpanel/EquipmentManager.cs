...
{
    [Header("Root Panel (same as itemDetailPanelX)")]
    public GameObject panelRoot;

    [Header("Window Pager (optional)")]
    public ItemDetailPager pager;   // your left/right flip script
...
public class EquipmentManager : MonoBehaviour
{
    ...
    [Header("Detail Panel UI (per rarity)")]
    public ItemDetailUI detailUICommon;
    public ItemDetailUI detailUIRare;
    public ItemDetailUI detailUILegendary;
    public ItemDetailUI detailUIExotic;
...
ItemDetailUI GetUIBundleForPanel(GameObject panel)
{
    if (panel == itemDetailPanelC) return detailUICommon;
    if (panel == itemDetailPanelR) return detailUIRare;
    if (panel == itemDetailPanelL) return detailUILegendary;
    if (panel == itemDetailPanelE) return detailUIExotic;
    return null;
}

static string ReadableCategory(EquipmentCategory c) => c switch
{
    EquipmentCategory.MeleeWeapon  => "Melee Weapon",
    EquipmentCategory.RangedWeapon => "Ranged Weapon",
    EquipmentCategory.Accessory    => "Accessory",
    _ => c.ToString()
};

static string ReadableRarity(EquipmentRarity r) => r switch
{
    EquipmentRarity.Common    => "Common",
    EquipmentRarity.Rare      => "Rare",
    EquipmentRarity.Legendary => "Legendary",
    EquipmentRarity.Exotic    => "Exotic",
    _ => r.ToString()
};
...
{
    // Pick the UI bundle that matches the active rarity panel
    var ui = GetUIBundleForPanel(currentActiveDetailPanel);
    if (ui == null) return;

    // Reset the flip window to the first page (moves). Remove if you want persistence.
    if (ui.pager != null) ui.pager.SetPage(0);

    // Header
    if (ui.itemNameText) ui.itemNameText.text = item.itemName ?? "";
    if (ui.typeText)     ui.typeText.text     = ReadableCategory(item.category);
    if (ui.rarityText)   ui.rarityText.text   = ReadableRarity(item.rarity);

    if (ui.portraitImage)
    {
        ui.portraitImage.sprite  = item.itemIcon;
        ui.portraitImage.enabled = (item.itemIcon != null);
    }

    // Moves (X/Z) — only meaningful for weapons. Show em dashes when missing.
    string xMove = "—";
    string zMove = "—";

    bool isWeapon = item.category == EquipmentCategory.MeleeWeapon ||
                    item.category == EquipmentCategory.RangedWeapon;

    if (isWeapon && item.moveset != null)
    {
        if (item.moveset.Length > 0 && !string.IsNullOrWhiteSpace(item.moveset[0])) xMove = item.moveset[0];
        if (item.moveset.Length > 1 && !string.IsNullOrWhiteSpace(item.moveset[1])) zMove = item.moveset[1];
    }

    if (ui.xMoveText) ui.xMoveText.text = xMove;
    if (ui.zMoveText) ui.zMoveText.text = zMove;

    // Description page
    if (ui.descriptionText) ui.descriptionText.text = item.itemDescription ?? "";

    // Keep your existing equip button logic
    UpdateEquipButton();
}
