...
public class SlabRingSimpleFX : MonoBehaviour
{
    ...
    [Tooltip("If ON, the effect shrinks after fading. If OFF, it just fades then optionally destroys.")]
    public bool shrinkAfterFade = false;
    ...
    /// <summary>Starts the effect (scale-in → hold → fade → optional shrink → optional destroy).</summary>
    public void Play()
    {
        StopAllCoroutines();
    ...
    private void CacheRenderers()
    {
        ...
        for (int i = 0; i < _renderers.Count; i++)
        {
            ...
            // Allocate/track an MPB
            var mpb = new MaterialPropertyBlock();
            if (r != null) r.GetPropertyBlock(mpb);
            _mpbs.Add(mpb);

            // Instantiate per-renderer materials and force them transparent so alpha fading works
            var mats = r ? r.materials : null; // material instances
            if (mats != null)
            {
                for (int m = 0; m < mats.Length; m++)
                {
                    TryMakeTransparent(mats[m]);
                }
                // re-assign to ensure instances stay on the renderer
                r.materials = mats;
            }

            _instancedMats.Add(mats);
        }
    }
    ...
    void SetAlpha(float a)
    {
        ...
        for (int i = 0; i < _renderers.Count; i++)
        {
            ...
            // Pull base color from the CURRENT instanced material so RGB tint matches
            ...
            // Apply to ALL submeshes so every slot fades
            int subCount = (r != null && r.sharedMaterials != null) ? r.sharedMaterials.Length : 1;
#if UNITY_2021_2_OR_NEWER
            for (int sub = 0; sub < subCount; sub++)
                r.SetPropertyBlock(mpb, sub);
#else
